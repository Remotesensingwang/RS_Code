...
pro dunhuangtest
  ...
  ;  out_directory='H:\data\FY3D\tifout\snowwatercloud\2021\'
  out_directory='H:\data\FY3D\testtif\removesnow\Yang\'
  ;文件日期 角度 匹配站点范围各个波段的toa均值
  ;openw,lun,'H:\data\toa\snowwatercloud\dh_toa2021_fy3d.txt',/get_lun,/append,width=500
  ...
  for file_i_hdf=0,file_n_hdf-1 do begin
    ...
    toa_094=TOAdata[*,*,17]
    b=(toa_red-toa_094)/(toa_red+toa_094)
    ;cloudpos=where((toa_nir gt 0.3)or(toa_red gt 0.3)or(b gt 0),/null)
    
    cloudpos=where(((toa_green_result.std ge 0.0025) and (toa_blue ge 0.4)) or ((toa_nir_result.std ge 0.0025) and (toa_nir ge 0.011)),/null)
    ...
    for layer_i=0,TOAdata_size[3]-2 do begin
...
;    printf,lun,strcompress(data,/remove_all);,format='(25(a,:,","))'
    ...
    result_tiff_name=out_directory+strmid(file_basename(file_list_hdf[file_i_hdf],'.hdf'),19,13)+'_TOA_removeall_Yang.tif'
