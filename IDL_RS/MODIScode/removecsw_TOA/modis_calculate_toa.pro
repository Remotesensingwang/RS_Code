...
pro modis_calculate_toa
  compile_opt idl2
  ;input_directory='H:\00data\MODIS\2019-1'
  input_directory='F:\MODISdata'
  out_directory='H:\00data\MODIS\tifout\removecsw\2019\'

  ;文件日期 角度 匹配站点范围各个波段的toa均值
  ;openw,lun,'H:\00data\toa\MODIS\snowwatercloud\2019\dh_dingbiao2019_modis.txt',/get_lun,/append,width=500
  openw,lun,'F:\MODISdata\dh_dingbiao2020_modis.txt',/get_lun,/append,width=500
  ;敦煌定标场中心坐标
  ...
  for file_i_hdf=0,file_n_hdf-1 do begin
    ...
    for layer_i=0,TOA_data_size[3]-1 do begin
      data=TOA_data[*,*,layer_i]
      data[cloudpos]=!VALUES.F_NAN
      TOA_data[*,*,layer_i]=data
    endfor
    ...
    result_tiff_name=out_directory+datetime+'_TOA_removecsw.tif'
